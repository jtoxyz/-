-- Non-destructive production schema consolidation.
-- Records the fixes currently active in Supabase while preserving all legacy
-- columns and existing features. No tables or compatibility columns are removed.
  ...
  IF NEW.ticket_code IS NULL OR btrim(NEW.ticket_code) = '' THEN
    LOOP
      ...
      EXIT WHEN NOT EXISTS (
        ...
        WHERE r.ticket_code = NEW.ticket_code
      );
    END LOOP;
  END IF;
...
-- Remove only the obsolete overload that caused PostgREST ambiguity.
...
BEGIN
  ...
  IF v_slot.reservation_starts_at IS NOT NULL
     AND now() < v_slot.reservation_starts_at THEN
    RAISE EXCEPTION '予約開始日時前です。';
  END IF;

  IF v_slot.reservation_ends_at IS NOT NULL
     AND now() > v_slot.reservation_ends_at THEN
  ...
  IF (v_reserved_count + v_pre_reserved_count) >=
     COALESCE(v_slot.reservation_capacity, v_slot.total_capacity) THEN
    RAISE EXCEPTION '予約枠の定員に達しました。';
  END IF;

  IF (v_reserved_count + v_pre_reserved_count +
      v_walkin_count + v_pre_walkin_count) >= v_slot.total_capacity THEN
...
-- Add only non-destructive indexes. These do not change behaviour.
