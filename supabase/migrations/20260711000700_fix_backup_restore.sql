-- Migration: 20260711_fix_backup_restore.sql
-- Fixes backup/restore logic to include admin_pre_registrations.

-- 1. Export Backup RPC
DROP FUNCTION IF EXISTS admin_export_event_backup(uuid, boolean);
...
BEGIN
  ...
  IF p_include_reservations THEN
    ...
    SELECT COALESCE(jsonb_agg(row_to_json(pre)), '[]'::jsonb) INTO v_pre_registrations
    FROM admin_pre_registrations pre
    WHERE pre.event_id = p_event_id;
  END IF;
  ...
  IF p_include_reservations THEN
    ...
    v_result := jsonb_set(v_result, '{pre_registrations}', v_pre_registrations);
  END IF;
...
DECLARE
  ...
  v_pre_registrations_data jsonb;
  v_slot jsonb;
  v_res jsonb;
  v_pre jsonb;
  v_old_slot_id uuid;
  ...
  IF v_backup_type = 'full' THEN
    ...
    IF v_reservations_data IS NOT NULL THEN
      ...
      LOOP
        ...
        -- Support old data missing some fields
        INSERT INTO reservations (
          event_id, event_slot_id, university_email, student_number, student_name,
          department, ticket_type, status, created_at, used_at, cancelled_at, public_token
        ) VALUES (
          v_new_event_id,
          v_new_slot_id,
          v_res->>'university_email',
          v_res->>'student_number',
          v_res->>'student_name',
          v_res->>'department',
          v_res->>'ticket_type',
          v_res->>'status',
          COALESCE((v_res->>'created_at')::timestamptz, now()),
    ...
    -- Restore pre-registrations
    v_pre_registrations_data := p_json->'pre_registrations';
    IF v_pre_registrations_data IS NOT NULL THEN
      FOR v_pre IN SELECT * FROM jsonb_array_elements(v_pre_registrations_data)
      LOOP
        v_old_slot_id := (v_pre->>'event_slot_id')::uuid;
        v_new_slot_id := (v_slot_mapping->>v_old_slot_id::text)::uuid;
        
        IF v_new_slot_id IS NULL THEN
          RAISE EXCEPTION 'Invalid event_slot_id % in pre_registrations data', v_old_slot_id;
        END IF;

        INSERT INTO admin_pre_registrations (
          event_id, event_slot_id, student_name, student_number, university_email,
          ticket_type, status, created_at
        ) VALUES (
          v_new_event_id,
          v_new_slot_id,
          v_pre->>'student_name',
          v_pre->>'student_number',
          v_pre->>'university_email',
          v_pre->>'ticket_type',
          v_pre->>'status',
          COALESCE((v_pre->>'created_at')::timestamptz, now())
        );
      END LOOP;
    END IF;
