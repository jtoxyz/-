type Worksheet = { rows: unknown[][] };
type Workbook = { sheets: Array<{ name: string; sheet: Worksheet }> };
...
function buildRows(data: unknown[][]): unknown[][] {
  ...
  const studentIndex = headers.indexOf('学籍番号');
  const slotIndex = headers.indexOf('開催枠');

  if (eventIndex < 0 || nameIndex < 0 || studentIndex < 0 || slotIndex < 0) {
    return data;
  }
  ...
  const sortedRows = [...sourceRows].sort((left, right) => {
    const slotResult = collator.compare(
    ...
    const nameResult = collator.compare(leftName, rightName);
    if (nameResult !== 0) return nameResult;

    return collator.compare(
      String(left?.[studentIndex] ?? ''),
      String(right?.[studentIndex] ?? ''),
    );
  });
...
export function writeFile(workbook: Workbook, requestedFileName: string): void {
  ...
  const html = `<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"><style>
    ...
    .title { background: #fff; font-size: 16pt; text-align: left; height: 30px; }
